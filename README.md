...
op item create --category "API Credential" --title "my-media-center-ci SA Token" --vault "<your vault>" "token[concealed]=$(echo "$OUT" | jq -r .token)" </dev/null

# 2. Credentials the app consumes (items referenced by .env.tpl / deploy.yml)
op item create --category "API Credential" --title "Notion API Key" --vault MediaCenter \
...
gh secret set OP_SERVICE_ACCOUNT_TOKEN --repo <owner>/<repo> --body "$(op read 'op://<your vault>/my-media-center-ci SA Token/token')"
```
